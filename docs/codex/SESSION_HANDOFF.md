...
- Done:
  ...
  - Kept the species count low and varied only local position, Y rotation, and relative scale so the visible size range stays around the intended old-scale 4.2x–6.0x band on top of the new 5x baseline wrappers.
...
- Next:
  - Reload `E:\山河志风起汉末\scenes\china_terrain_30km.tscn`, inspect Luoyang again, and if the grove density/size feels right, convert this two-species setup into Terrain3D instancer data for wider regional planting.
...
- Done:
  - Rechecked `E:\ɽ��־����ĩ\scenes\china_terrain_30km.tscn` and confirmed the grass/rock ext_resources had drifted back to the old `nature/textures` set instead of the intended packed terrain textures.
  - Rebound the scene's 4 Terrain3D texture slots to the packed texture set under `E:\ɽ��־����ĩ\�ز�\textures\`: grass=`rocky_terrain_02_packed_*`, soil=`coast_sand_rocks_02_packed_*`, rock=`gravelly_sand_packed_*`, snow=`snow_02_packed_*`.
...
- Risks:
  ...
  - There is still an unrelated script parse failure in `E:\ɽ��־����ĩ\scripts\presentation\strategy_ui_layer.gd` referenced by `china_terrain_scene.gd`; this is not the white-texture root cause but may confuse editor refresh behavior.
- Next:
  - Reopen `E:\ɽ��־����ĩ\scenes\china_terrain_30km.tscn` fresh in Godot 3D view, verify the terrain is no longer white, and only then continue tuning tiling/color parameters.
...
- Done:
  - Updated `E:\山河志风起汉末\scripts\presentation\tree_batch_light_tuner.gd` to support explicit `leaf_tint` and `bark_tint`, so large-map tree wrappers can push species-specific color contrast without touching the source imported materials.
  - Tuned `E:\山河志风起汉末\scenes\prefabs\vegetation\black_pine_tree_b_batch_light.tscn` toward darker cooler foliage, lower alpha scissor, and double-sided leaves so the green trees read fuller and stand apart from the grass.
  - Tuned `E:\山河志风起汉末\scenes\prefabs\vegetation\sakura_tree_a_batch_light.tscn` toward brighter pink foliage, lower alpha scissor, and double-sided leaves so sakura stands out more clearly.
  - Rebuilt `E:\山河志风起汉末\scenes\prefabs\vegetation\japanese_tree_cluster_luoyang.tscn` into a denser grove with 30 trees total (18 green + 12 sakura) arranged in tighter local clumps around Luoyang.
...
- Next:
  - Reload `E:\山河志风起汉末\scenes\china_terrain_30km.tscn` and judge whether the darker pine + brighter sakura + denser clumps are now visible enough; if still not, the next step should be reducing local grass around the grove area or adding a small contrasting ground patch under tree groups.
...
- Done:
  - Updated E:\ɽ��־����ĩ\scripts\presentation\strategy_ui_layer.gd expedition picker table from Officer/Battle/Tactic/Stats/Special to Officer/Battle/Lead/Might/Int/Pol/Charm/Special and widened the picker panel so the secondary menu is closer to the primary panel footprint.
  ...
  - Recorded the new UI rule in E:\ɽ��־����ĩ\docs\codex\PROJECT_MEMORY.md: officer overview should be reusable, five stats should split into separate fields when space allows, and Battle should be preferred over Tactic in default officer-overview UIs.
  - Verified the focused headless parse still succeeds with E:\SteamLibrary\steamapps\common\Godot Engine\godot.windows.opt.tools.64.exe --headless --path E:\ɽ��־����ĩ -s res://tmp/_tmp_verify_expedition_parse.gd.
- Risks:
...
- Done:
  - Updated `E:\ɽ��־����ĩ\scenes\china_terrain_30km.tscn` Terrain3D anti-tiling noise from a cellular-style pattern to a smoother continuous pattern by changing the `FastNoiseLite` setup.
...
- Done:
  - Updated `E:\山河志风起汉末\scenes\prefabs\vegetation\black_pine_tree_b_batch_light.tscn` so the built-in green tree base scale is now `Vector3(10, 10, 10)` instead of `Vector3(5, 5, 5)`.
...
- Next:
  - Reload `E:\山河志风起汉末\scenes\china_terrain_30km.tscn` and judge whether the enlarged green canopy is now readable enough against the grass.
...
- Done:
  - Updated `E:\ɽ��־����ĩ\scenes\china_terrain_30km.tscn` Terrain3D material to reduce layer transition sharpness further by lowering `blend_sharpness` to `0.12`.
...
- Done:
  - Relaxed the green tree tint in `E:\山河志风起汉末\scenes\prefabs\vegetation\black_pine_tree_b_batch_light.tscn` so the canopy is still visible but less aggressively dark/saturated.
  - Relaxed the sakura tint in `E:\山河志风起汉末\scenes\prefabs\vegetation\sakura_tree_a_batch_light.tscn` so the pink reads softer and less punchy.
  - Added a modest outer ring to `E:\山河志风起汉末\scenes\prefabs\vegetation\japanese_tree_cluster_luoyang.tscn`: 6 extra green trees and 4 extra sakura.
...
- Next:
  - Reload `E:\山河志风起汉末\scenes\china_terrain_30km.tscn` and judge whether the colors now feel softer while the grove is slightly fuller.
...
- Done:
  - Rechecked `E:\ɽ��־����ĩ\scripts\presentation\strategy_ui_layer.gd` and confirmed the top-level cloud icon / aide portrait constants had mojibake-corrupted paths.
  - Replaced them with valid project paths: `res://�ز�/icon/xiangyun.png` and `res://�ز�/����`.
  - Re-ran a headless load for `E:\ɽ��־����ĩ\scenes\china_terrain_30km.tscn`; the scene now loads cleanly without the earlier `StrategyUILayer` resolution noise.
- Risks:
...
- Done:
  ...
  - Added an initial LFS tracking plan in `E:\山河志风起汉末\.gitattributes` and committed it as `3ca3427 chore: add initial lfs tracking rules`.
  - Scoped the first-pass LFS rules to the currently untracked large asset trees most clearly tied to runtime use: `assets_imports/` heavy binary media, `data/terrain/china_30km` height/terrain region payloads, and selected runtime-facing subtrees under `素材/`.
- Risks:
  ...
  - The LFS rules are intentionally conservative-first; before tracking the entire raw asset universe, verify which `素材/` subtrees are truly required for distributable runtime history.
- Next:
...
- Done:
  - Updated `E:\ɽ��־����ĩ\scripts\presentation\strategy_ui_layer.gd` expedition setup flow to ingest `panel_data["assignments"]`, rebuild the officer->office map, restore saved `selected_officer_ids`, and auto-sort them into commander order before padding the three visible slots.
  ...
  - Verified the focused headless parse succeeds with `E:\SteamLibrary\steamapps\common\Godot Engine\godot.windows.opt.tools.64.exe --headless --path E:\ɽ��־����ĩ -s res://tmp/_tmp_verify_expedition_parse.gd`.
- Risks:
...
- Done:
  - Updated `E:\ɽ��־����ĩ\scripts\presentation\strategy_ui_layer.gd` so the expedition picker table no longer prepends `[Main] / [Vice I] / [Vice II]` to officer names; commander ordering remains automatic but is now implicit through the top slot cards only.
  ...
  - Re-verified the focused parse path with `E:\SteamLibrary\steamapps\common\Godot Engine\godot.windows.opt.tools.64.exe --headless --path E:\ɽ��־����ĩ -s res://tmp/_tmp_verify_expedition_parse.gd`.
- Risks:
...
- Risks:
  ...
  - Other large trees such as `assets_imports/` and `素材/` remain outside version control; adding them to plain Git should be done carefully in smaller runtime-focused batches.
- Next:
  - If continuing on Gitee, stage the next runtime-critical asset batch in plain Git (recommended: only the exact `assets_imports/rpg_troops` subset referenced by `battle_rules.json`), or switch future large-asset storage to a Git LFS-capable remote before adding `素材/`.
...
- Done:
  - Updated `E:\ɽ��־����ĩ\scripts\presentation\strategy_ui_layer.gd` to connect expedition picker table `gui_input` and added `_toggle_expedition_picker_officer(...)`, so left-clicking a row now directly toggles that officer in/out of the current three-officer team without using the bottom button to add/remove.
...
- Done:
  - Updated `E:\ɽ��־����ĩ\scripts\battlefield_controller.gd` loss-report routing for player-side unit defeat branches.
  - Added helpers to separate relay HUD lines from unit self-lines: player loss events now push the left report entry, then show bottom HUD speaker `����`, then queue one short first-person line from the defeated friendly unit.
  - Converted player-side bad-news wording from commander-style `�������Ҿ�...` voice to relay-style `������...` for unit defeat, site loss, and facility destruction loss branches.
  - Kept player-side victory branches on commander voice, so only bad-news routing changed.
  - Re-validated `battlefield_controller.gd` parses and `E:\ɽ��־����ĩ\scenes\china_terrain_30km.tscn` headless-loads again.
- Risks:
  ...
  - The first-person line currently uses fallback text through `pick_commander_dialogue_line("defeat_unit", ...)`; if you later want named generals to have bespoke defeat/capture quotes, add that event key in `E:\ɽ��־����ĩ\scripts\unit_controller.gd` dialogue tables.
- Next:
  - Play one battle where a friendly unit is destroyed and confirm the sequence reads as: left report `����...`, bottom HUD speaker `����`, then a short first-person defeated-unit line; after that, wire the same pattern into officer-capture once the event source is identified.
...
- Done:
  ...
  - Tightened `E:\山河志风起汉末\.gitignore` with `assets_imports/**/_tmp_*` so imported scratch outputs stop polluting future Git status scans.
...
- Next:
  - If continuing resource onboarding, do the next batch either from `assets_imports/mixamo_troops/out` (runtime-ready character assets) or from a tightly scoped `素材/` runtime subset such as `素材/icon`, `素材/ui`, and directly referenced VFX textures.

## 2026-03-25 00:07 (Asia/Shanghai)
- Owner: Codex
- Goal:
  - Make the expedition officer picker behave like a true secondary menu inside the same primary command panel: same footprint as theһ���˵�, and reuse the primary confirm/cancel buttons instead of adding a separate bottom button row.
- Done:
  - Updated `E:\ɽ��־����ĩ\scripts\presentation\strategy_ui_layer.gd` so the expedition picker overlay now expands to the full command-panel footprint and the picker content area is laid out against the same panel bounds instead of using the previous smaller floating subpanel.
  ...
  - Re-verified with `E:\SteamLibrary\steamapps\common\Godot Engine\godot.windows.opt.tools.64.exe --headless --path E:\ɽ��־����ĩ -s res://tmp/_tmp_verify_expedition_parse.gd`.
- Risks:
...
- Done:
  - Updated `E:\ɽ��־����ĩ\scripts\battlefield_controller.gd` so the player-side unit-defeat branches no longer enqueue `����` when a first-person defeated-unit line is already queued.
  - Kept the left battle report entry intact; only the bottom HUD sequence changed.
  - Revalidated `battlefield_controller.gd` parse and `E:\ɽ��־����ĩ\scenes\china_terrain_30km.tscn` headless load.
- Risks:
...
- Next:
  - Trigger one friendly unit defeat in battle and confirm the bottom HUD now shows only the unit's first-person line instead of `���� + ����`.
