# Copyright 漏 2025 Cory Petkovsek, Roope Palmroos, and Contributors.
# UI for Terrain3D
extends Node
...
const FORCE_HIDE_EDITOR_DECAL: bool = true
...
func hide_decal() -> void:
	_clear_decal_shader_params()


func _clear_decal_shader_params() -> void:
	reset_decal_arrays()
	if editor_decal_timer != null:
		editor_decal_timer.stop()
	if plugin != null and plugin.terrain != null and plugin.terrain.material != null:
		mat_rid = plugin.terrain.material.get_material_rid()
	editor_decal_position = [Vector2.ZERO, Vector2.ZERO, Vector2.ZERO]
	editor_decal_rotation = [0.0, 0.0, 0.0]
	editor_decal_size = [0.0, 0.0, 0.0]
	editor_decal_color = [Color(0, 0, 0, 0), Color(0, 0, 0, 0), Color(0, 0, 0, 0)]
	editor_decal_visible = [false, false, false]
	editor_brush_texture_rid = RID()
	editor_decal_fade = 0.0
	if is_shader_valid():
		var r_map: PackedInt32Array = plugin.terrain.data.get_region_map()
		RenderingServer.material_set_param(mat_rid, "_editor_brush_texture", editor_brush_texture_rid)
		RenderingServer.material_set_param(mat_rid, "_editor_decal_position", editor_decal_position)
		...
		RenderingServer.material_set_param(mat_rid, "_editor_decal_color", editor_decal_color)
		RenderingServer.material_set_param(mat_rid, "_editor_decal_visible", editor_decal_visible)
		RenderingServer.material_set_param(mat_rid, "_editor_crosshair_threshold", -1.0)
		RenderingServer.material_set_param(mat_rid, "_region_map", r_map)
